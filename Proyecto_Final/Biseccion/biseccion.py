...
def result(x_l, x_r, tol):
    ...
    while candidate > tol:
        ...
        if candidate < tol:
            ...
            test = eval_func(x_l) * fm
            if test > 0:
                x_l = x_m 
            else:
                x_r = x_m
    final = "Resultado en: "+ str(x_m)
    global raiz
    nombreLabel=Label(raiz,text=final)
    nombreLabel.place(x=10, y=500)
    return x_m
...
def boton():
    try:   
        ...
        textito = "Las raices de tu función son: " + str(result(infe,supe,0.001))
...
def biseccion():
    ...
    global raiz
    raiz = Toplevel()
    raiz.title("Biseccion")
    raiz.resizable(False,False)
    raiz.geometry("600x450")
    
    raiz.config(bg="deepskyblue4")
    
    label = Label(raiz,text="Método de Bisección")
    label.pack(anchor=CENTER)
    label.config(fg="snow", #color de letra
                 bg="deepskyblue4", #fondo para todas la GUI
