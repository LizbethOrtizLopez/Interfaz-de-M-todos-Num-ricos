...
def LINEARIZACION_BASE10(x,y,n):
    try:
        sumy_log_d = 0
        sumx_log_d = 0
        sumxy_log_d = 0
        ...
        for i in range (n):
            sumx_log_d = sumx_log_d + math.log10(x[i])
            sumy_log_d = sumy_log_d + math.log10(y[i])
            sumxy_log_d = sumxy_log_d + math.log10(x[i]) * math.log10(y[i])
            sumx2_log_d = sumx2_log_d + math.log10(x[i]) * math.log10(x[i])
        a1 = ((n*(sumxy_log_d)) - (sumx_log_d * sumy_log_d)) / (n * sumx2_log_d - (sumx_log_d)**2)
        a0 = (sumy_log_d/n) - (a1 * (sumx_log_d/n))
        a2 = 10**a0
        return a1,a2
...
def leerArchivo(file,n):
    try:
        ...
        for i in range(n):
            if (float(row[i])==0.0):
                messagebox.showinfo("ERROR FATAL","Error. Ningún valor puede ser 0")
                raise Exception("Error. Ningún valor puede ser 0")
...
def boton():
    ...
    global n
    integer_n = int(n.get())
    b, a = LINEARIZACION_BASE10(x,y,integer_n)
...
def re_log10():
    ...
    global raiz
    raiz = Toplevel()
    raiz.title("Linearización con log10")
    raiz.resizable(False,False)
    raiz.geometry("500x400")
    
    raiz.config(bg="deepskyblue4")
    
    label = Label(raiz,text="Linearización con logaritmo base 10")
    label.pack(anchor=CENTER)
    label.config(fg="snow", #color de letra
                 bg="deepskyblue4", #fondo para todas la GUI
                 font=("Courier",16)) #tipo de letra
    ...
    label = Label(raiz,text="Precaución: Ningún valor puede ser 0")
    label.pack(anchor=CENTER)
    label.config(fg="snow", #color de letra
                 bg="deepskyblue4", #fondo para todas la GUI
                 font=("Courier",11)) #tipo de letra
