...
def rk4(x,y,h):
    k_1 = eval_func(x,y)
    y_m = y + k_1 * (h/2)
    k_2 = eval_func(x + (h/2),y_m)
    y_m = y + k_2 * (h/2)
    k_3 = eval_func(x + (h//2),y_m)
    y_m = y + k_3 * h
    k_4 = eval_func(x + h,y_m)
    slope = (k_1+2*(k_2+k_3)+k_4)/(6)
    y_new = y + slope * h
    x_new = x + h
    return (x_new,y_new)
...
def rk4_main():
    ...
    global raiz
    raiz = Toplevel()
    raiz.title("Método de Runge Kutta de cuarto orden")
    raiz.resizable(False,False)
    raiz.geometry("600x550")
    
    raiz.config(bg="deepskyblue4")
    
    label = Label(raiz,text="Método de Runge Kutta de cuarto orden")
    label.pack(anchor=CENTER)
    label.config(fg="snow", #color de letra
                 bg="deepskyblue4", #fondo para todas la GUI
