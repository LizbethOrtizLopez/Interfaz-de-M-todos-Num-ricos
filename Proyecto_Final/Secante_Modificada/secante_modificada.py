...
def secanteM(x0, maxiter, delta, ed):
    xold = x0
    iter = 0
    ea = 101
    while iter < maxiter and ea > ed:
        xnew = xold - ((delta*xold*(eval_func(xold))) / (eval_func((xold+(delta*xold)))-eval_func(xold)))
        if math.fabs(xnew) > 0:
...
def boton(): 
    try:    
        ...
        if (x == 0):
            messagebox.showinfo("ERROR FATAL","Error. No puede haber elementos 0")
            raise Exception("Error. No puede haber elementos 0")
        ...
        textito = "Las raices de tu función son: " + str(secanteM(x,maxi_num,0.01,0.001))
...
def sec_m():
    ...
    global raiz
    raiz = Toplevel()
    raiz.title("secante Modificada")
    raiz.resizable(False,False)
    raiz.geometry("600x450")
    
    raiz.config(bg="deepskyblue4")
    
    label = Label(raiz,text="Secante Modificada")
    label.pack(anchor=CENTER)
    label.config(fg="snow", #color de letra
                 bg="deepskyblue4", #fondo para todas la GUI
    ...
    label = Label(raiz,text="Ingresa x0:")
    label.pack(anchor=CENTER)
    label.config(fg="snow", #color de letra
                 bg="deepskyblue3", #fondo para todas la GUI
                 font=("Courier",11)) #tipo de letra
